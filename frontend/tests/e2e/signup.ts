import { browser, by, element, env , expect, logout, USER } from "./config/imports";
...
describe("Sign up page ", () => {
  ...
  describe("should have sign up box", () => {
      ...
      it("with a sign up button", () => {
        ...
        expect(checkbox.all(by.css(".sign-up-button")).first().isPresent()).to.eventually.equal(true);
      });

      it("with a Log in link", () => {
        expect(checkbox.all(by.css(".logIn")).count()).to.eventually.equal(1);
        expect(checkbox.all(by.css(".logIn")).first().isPresent()).to.eventually.equal(true);
      });

      it("with a Pricing link", () => {
        expect(checkbox.all(by.css(".pricing")).count()).to.eventually.equal(1);
        expect(checkbox.all(by.css(".pricing")).first().isPresent()).to.eventually.equal(true);
      });

      it("trying to login without any field filled in fails", () => {
        const register = element(by.css('[ng-click="vm.register()"]'));
        expect(register.isDisplayed()).to.eventually.equal(true);
        register.click();
        browser.sleep(500);
