...
(function() {
	...
	Viewer = function(name, element, manager, callback, errCallback) {
		...
		self.gyroCallback = function (event) {
			self.gyroOrientation = event;
		};

		self.startGyro = function () {
			self.runtime.enterFrame = function () {
				if (self.gyroOrientation)
				{
					self.gyroscope(
			...
			if(window.DeviceOrientationEvent){
				window.addEventListener("deviceorientation", self.gyroCallback, false);
			}
		};

		self.endGyro = function () {
			window.removeEventListener("deviceorientation", self.gyroCallback);
		}
